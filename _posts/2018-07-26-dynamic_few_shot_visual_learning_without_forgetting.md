...
where $\phi_q \in \mathbb R^{d\times d}$ is a learnable weight matrix that transforms the feature vector to query vector used for querying the memory, $\{ k_b \in \mathbb R^d \}_b^{K_{base}} $ is a set of $K_{base} $ learnable keys (one per base category) used for indexing the memory and $Att(\cdot, \cdot)$.
